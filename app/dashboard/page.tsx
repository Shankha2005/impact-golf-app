...
export default async function DashboardHome() {
  ...
  try {
    const [subRes, charityRes, participationsRes] = await Promise.all([
      ...
      supabase
        ...
        .eq('user_id', userId)
        .maybeSingle(),
      supabase.from('draw_participations').select('draw_id').eq('user_id', userId),
    ]);

    drawsEntered = participationsRes.error ? 0 : (participationsRes.data?.length ?? 0);
    charityName = charityNameFromPreference(charityRes?.data ?? null);
    contributionPercent =
    ...
    console.error('[dashboard/page] summary fetch', e);
  }
  ...
  try {
    ...
    nextDrawLabel = nextDraw.toLocaleDateString('en-US', { month: 'long', day: 'numeric' });
  } catch (e) {
    console.error('[dashboard/page] next draw date', e);
  }
    ...
    <div className="max-w-4xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-charity-light p-6 rounded-xl border border-green-200 shadow-sm">
          ...
          <p className="text-3xl font-bold text-green-900 mt-2">{nextDrawLabel}</p>
        </div>
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
          <h3 className="text-gray-500 text-sm font-medium">Subscription</h3>
          <p className="text-2xl font-bold text-gray-900 mt-2 capitalize">{subscriptionLabel}</p>
        </div>
      </div>
      ...
      <div className="bg-white p-8 rounded-xl border border-gray-100 shadow-sm">
        ...
        <p className="text-gray-600 mb-4">
          Your subscription contributes <strong>{contributionPercent}%</strong> to <strong>{charityName}</strong>. Your
          support helps make a real difference.
        </p>
